...
namespace GMS
{
    public partial class EditEmployeeProfileForm : Form
    {
        ...
        private void EditEmployeeProfileForm_Load(object sender, EventArgs e)
        {

        }

        private void updateEmployeeEmailTextBox_TextChanged(object sender, EventArgs e)
        {
...
namespace GMS
{
    public partial class EditEmployeeProfileForm : Form
    {
        ...
        SqlConnection sq = new SqlConnection("Data Source=DESKTOP-35F3HM7\\SQLEXPRESS01;Initial Catalog=table_1;Persist Security Info=True;User ID=sa;Password=root");
        private void updateEmployeeEnterBtn_Click(object sender, EventArgs e)
        {
            try
            {
                if (updateEmployeeNameTextBox.Text == "" && updateEmployeePasswordTextBox.Text == "" && updateEmployeeEmailTextBox.Text == "" && updateEmployeePhoneTextBox.Text == "" && updateEmployeeAddressTextBox.Text == "")
                {
                    MessageBox.Show("Recheck your information");
                }
                else
                {
                    SqlCommand cmd = new SqlCommand("UPDATE emp SET name = @name, password = @password, email = @email, phone = @phone, address = @address WHERE eid = '" + this.eid + "'", sq);
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.AddWithValue("@name", updateEmployeeNameTextBox.Text);
                    cmd.Parameters.AddWithValue("@password", updateEmployeePasswordTextBox.Text);
                    //cmd.Parameters.AddWithValue("@salary", updateEmployeeSalaryTextBox.Text);
